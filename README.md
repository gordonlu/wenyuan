...
下面这些能力可以进入“行为契约冻结”状态：后续可以优化 UI、文案和性能，但不应随意改接口语义、数据结构或主流程阶段。

- 三席合议主流程：独议、批议、复议、阁议、一次合案复议、最终多数/无多数结果。
- Single Agent 对照模式：作为三席模式的基线对照保留。
...
- 导出层级：Markdown brief/standard/audit、JSON、HTML 保留为稳定导出方向。

下面这些还不应冻结，需要继续产品化：

- 运行中席位卡动画、工具执行态和状态条。
- 结果可信度摘要和证据来源摘要。
- 新建议题页的一键模板、来源区折叠和新用户引导。
- 工具轨迹的用户可读摘要。
- 错误/空状态的修复路径提示。
...
面向外部 Agent、自动化脚本或其他应用调用文渊阁的说明见 `README_FOR_AGENTS.md`；`review.md` 是下一阶段产品和 UI 编码大纲。
